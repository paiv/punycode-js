...
const testdata = [
    ...
    ["АбВґ'AbCd", "'AbCd-mqeo1wz4a"],
    ["𐌾𐌰𐌷𐌰𐌽𐌰𐌰𐌹𐍂𐌸𐌰𐌹", "ec8caaaa2bkhc8ah9c"],
...
for (let pair of testdata) {
    const s = pair[0]
    const u = pair[1]
    const x = punycode.encode(s)
    assert(x === u, 'got', JSON.stringify(x), 'expecting', JSON.stringify(u))
    const r = punycode.decode(x)
    assert(r === s, 'got', JSON.stringify(r), 'expecting', JSON.stringify(s))
}


function expect_throw(block, message) {
    try {
        block()
        assert(false, 'expecting to throw', JSON.stringify(message))
    }
    catch (err) {
        assert(err.message && err.message.startsWith(message), 'got', err, 'expecting', JSON.stringify(message))
    }
}


expect_throw(() => punycode.decode('йо-'), 'invalid char in basic string: ')
expect_throw(() => punycode.decode('-йо'), 'invalid char in delta encoding: ')
